...
| `carousel_product` (Carousel Produk) | `mode` manual\|filter; `filter?` {search, category, minPrice, maxPrice, flashSale, sortOrder}; `limit?` 1–20 default 10 | Manual = pilih produk (junction table, drag order). Filter = dinamis saat render (auto-update kalau produk berubah). |
...
8. Carousel filter: mode filter → set {category, sortOrder, limit} → Save. Store render produk dinamis sesuai filter.
