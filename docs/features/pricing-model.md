...
- `product_variants.discount` — the **raw `disc%`** written by the former SOH
  master-data sync (now null under Jubelio). It is preserved as-is (mixed
  int/decimal text) for future use but is **not** used for display, filtering,
  or discount derivation. The displayed discount comes solely from `base_price`
  vs `variant.price`.
