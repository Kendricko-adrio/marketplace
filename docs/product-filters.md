...
CMS (carousel "filter" mode + promo cards). Built on top of the SOH-synced
product master data — specifically the normalized `brand` and `gender`
dimensions — plus the already-supported sort and "on sale" (hasDiscount) filters.

## Why

The SOH import (migration `0004`) added free-text `brand` / `gender` / `season` /
`collection` / `articleNumber` fields to products, but none were selectable in
...
- **Sync-managed, no admin CRUD**: brand/gender rows are auto-created by the SOH
  CSV import and the `POST /api/webhooks/soh` webhook (the modal `Brand` / `sex`
  value per ART, upserted by slug). There is no admin brand/gender management
  page — the dimension list is owned by the supplier sync, matching the
  push-managed replica design (see [soh-sync/README.md](./soh-sync/README.md)).
...
- SOH sync (`packages/db/src/soh-sync.ts`): `upsertSohRecords` now upserts
  `brands` / `genders` **before** products (FK order), by slug, with deterministic
  ids (`keyId("soh:brand:", slug)`), then sets `products.brandId` / `genderId`
  (insert + on-conflict update). Both the CSV import and the webhook flow through
  this one function.
...
npm run db:import-soh                     # idempotent; (re)creates brands/genders + sets brandId/genderId
```
...
- Admin: edit a `carousel_product` section in filter mode with brand + gender +
  ...
  results.
- SOH: re-run `npm run db:import-soh` → `brand` / `gender` tables repopulate,
  `products.brand_id` / `gender_id` set; `npx tsx packages/db/src/soh-scan.ts`
  dry-run reports brand/gender counts with zero slug duplicates.
